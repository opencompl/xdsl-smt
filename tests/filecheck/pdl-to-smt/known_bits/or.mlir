...
builtin.module {
  pdl.pattern @addi_analysis : benefit(0) {
    ...
    %lhs_zeros, %lhs_ones = "pdl.dataflow.get"(%lhs) {"domain_name" = "known_bits"} : (!pdl.value) -> (!transfer.integer, !transfer.integer)
    %rhs_zeros, %rhs_ones = "pdl.dataflow.get"(%rhs) {"domain_name" = "known_bits"} : (!pdl.value) -> (!transfer.integer, !transfer.integer)
