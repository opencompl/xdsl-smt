// RUN: xdsl-smt "%s" -p=pdl-to-smt,lower-effects -t smt | z3 -in | filecheck "%s"
...
builtin.module {
  pdl.pattern @addi_analysis : benefit(0) {
    ...
    %lhs_zeros, %lhs_ones = "pdl.dataflow.get"(%lhs) {"domain_name" = "known_bits"} : (!pdl.value) -> (!transfer.integer<8>, !transfer.integer<8>)
    %rhs_zeros, %rhs_ones = "pdl.dataflow.get"(%rhs) {"domain_name" = "known_bits"} : (!pdl.value) -> (!transfer.integer<8>, !transfer.integer<8>)
