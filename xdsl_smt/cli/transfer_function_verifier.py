...
forward_soundness_check,backward_soundness_check,valid_abstract_domain_check, counterexample_check, int_attr_check
)
...
def get_dynamic_concrete_function(concrete_func_name: str, width: int, intAttr:dict[int,int], is_forward:bool) -> FuncOp:
    result =None
    ...
    if concrete_func_name == "comb_extract":
        ...
        combOp=comb.ExtractOp(result.args[0], IntegerAttr.from_int_and_width(low_bit, 64), resultIntTy)
    returnOp = Return(combOp.results[0])
    result.body.block.add_ops([combOp, returnOp])
    assert result is not None and ("Cannot find the concrete function for" + concrete_func_name)
    return result
    ...
    for k in comb_semantics.keys():
        if k.name == concrete_op_name:
            ...
            if concrete_op_name == "comb.mux":
                ...
                result = FuncOp(func_name, funcTy)
                if issubclass(k, comb.VariadicCombOperation) or concrete_op_name == "comb.concat":
...
def get_dynamic_transfer_function(func:FuncOp, width:int, module:ModuleOp, int_attr:dict[int,int])->DefineFunOp:
    ...
    new_function_type=FunctionType.from_lists(new_args_type, func.function_type.outputs)
...
def verify_smt_transfer_function(smt_transfer_function:SMTTransferFunction,domain_constraint:FunctionCollection,
                                 instance_constraint:FunctionCollection, width:int,
                                 transfer_function:TransferFunction,
                                 func_name_to_func:dict[str, FuncOp],
                                 dynamic_transfer_functions:dict[str, Func]) -> None:
    # Soundness check
    ...
    while True:
        query_module = ModuleOp([])
        added_ops = int_attr_check(smt_transfer_function, domain_constraint, instance_constraint, int_attr)
        ...
        if not verify_pattern(ctx, query_module):
            ...
            if smt_transfer_function.concrete_function is None:
                ...
                dynamic_concrete_function_module.body.block.add_ops(
                    [get_dynamic_concrete_function(smt_transfer_function.concrete_function_name, width, int_attr, transfer_function.is_forward)])
                lowerToSMTModule(dynamic_concrete_function_module, width, ctx)
                assert len(dynamic_concrete_function_module.ops) == 1
                assert isinstance(dynamic_concrete_function_module.ops.first, DefineFunOp)
            ...
            if smt_transfer_function.transfer_function is None:
                ...
                smt_transfer_function.transfer_function=get_dynamic_transfer_function(
                    dynamic_transfer_functions[smt_transfer_function.transfer_function_name],
                    width,dynamic_transfer_function_module, int_attr)
            ...
            if smt_transfer_function.soundness_counterexample is not None:
                ...
                added_ops = counterexample_check(func_name_to_func[counterexample_func_name],
                                                 smt_transfer_function.soundness_counterexample, domain_constraint,
                                                 int_attr)
                query_module.body.block.add_ops(added_ops)
                FunctionCallInline(True, {}).apply(ctx, query_module)
                LowerToSMT().apply(ctx, query_module)
    ...
    for func in module.ops:
        if isinstance(func, FuncOp):
            ...
            if is_transfer_function(func):
                transfer_functions[func_name] = TransferFunction(func, is_forward(func),
                                                                 get_operationNo(func), need_replace_int_attr(func))
    ...
    for width in solveVectorWidth():
        ...
        for op in smt_module.ops:
            ...
            if isinstance(op, FuncOp) and "applied_to" in op.attributes:
                ...
                if "int_attr" in op.attributes:
                    transfer_function_name_to_concrete_function_name[func_name] = get_dynamic_concrete_function_name(concrete_func_name)
                    continue
        ...
        for func_name, transfer_function in transfer_functions.items():
            ...
            if 'int_attr' in transfer_function.transfer_function.attributes:
                assert 'int_attr_constraint' in transfer_function.transfer_function.attributes
                int_attr_arg=get_int_attr_arg(transfer_function.transfer_function)
                int_attr_constraint= func_name_to_smt_func[
                    transfer_function.transfer_function.attributes['int_attr_constraint'].data]

            smt_transfer_function_obj = SMTTransferFunction(func_name, smt_transfer_function,
                                                        transfer_functions,
                                                        concrete_func_name,
                                                        concrete_func,
                                                        abs_op_constraint,
                                                        op_constraint,
                                                        soundness_counterexample, int_attr_arg, int_attr_constraint)
            verify_smt_transfer_function(smt_transfer_function_obj, domain_constraint,
                                         instance_constraint,width,transfer_function,func_name_to_func,
