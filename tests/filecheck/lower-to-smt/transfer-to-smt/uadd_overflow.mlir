...
// RUN: xdsl-smt %s -p=resolve-transfer-widths{width-map=\"default=8\"},lower-to-smt,lower-effects,smt-expand,canonicalize,dce,merge-func-results -t=smt | z3 -in

"builtin.module"() ({
  "func.func"() ({
  ^0(%x : !transfer.integer<@W>, %y : !transfer.integer<@W>):
    %r = "transfer.uadd_overflow"(%x, %y) : (!transfer.integer<@W>,!transfer.integer<@W>) -> !transfer.integer<1>
    "func.return"(%r) : (!transfer.integer<1>) -> ()
  }) {"sym_name" = "test", "function_type" = (!transfer.integer<@W>, !transfer.integer<@W>) -> !transfer.integer<1>, "sym_visibility" = "private"} : () -> ()
}) : () -> ()

// CHECK:      builtin.module {
// CHECK-NEXT:   %0 = "smt.define_fun"() ({
// CHECK-NEXT:   ^0(%x : !smt.bv<8>, %y : !smt.bv<8>, %1 : !effect.state):
// CHECK-NEXT:     %2 = "smt.bv.uaddo"(%x, %y) : (!smt.bv<8>, !smt.bv<8>) -> !smt.bool
