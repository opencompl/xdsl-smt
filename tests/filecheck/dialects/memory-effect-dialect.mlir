...
builtin.module {
    ...
    %size = "smt.bv.constant"() {value = #smt.bv<32> : !smt.bv<64>} : () -> !smt.bv<64>
    %state2, %ptr = mem_effect.alloc %state, %size
    %offset = "smt.bv.constant"() {value = #smt.bv<8> : !smt.bv<64>} : () -> !smt.bv<64>
    %new_ptr = mem_effect.offset_ptr %ptr[%offset]
    %val = "smt.bv.constant"() {value = #smt.bv<42> : !smt.bv<16>} : () -> !smt.bv<16>
...
// CHECK-NEXT:    %size = "smt.bv.constant"() {value = #smt.bv<32> : !smt.bv<64>} : () -> !smt.bv<64>
// CHECK-NEXT:    %state2, %ptr = mem_effect.alloc %state, %size
// CHECK-NEXT:    %offset = "smt.bv.constant"() {value = #smt.bv<8> : !smt.bv<64>} : () -> !smt.bv<64>
// CHECK-NEXT:    %new_ptr = mem_effect.offset_ptr %ptr[%offset]
// CHECK-NEXT:    %val = "smt.bv.constant"() {value = #smt.bv<42> : !smt.bv<16>} : () -> !smt.bv<16>
