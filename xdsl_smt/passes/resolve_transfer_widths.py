...
def resolve_transfer_widths(attr: Attribute, width_map: dict[str, int]) -> Attribute:
    ...
    if isinstance(attr, ArrayAttr):
        ...
        new_data: list[Attribute] = [
            resolve_transfer_widths(a, width_map) for a in data
        ]
    ...
    if isinstance(attr, DictionaryAttr):
        ...
        for k, v in attr.data.items():
            new_v = resolve_transfer_widths(v, width_map)
            if new_v is not v:
                changed = True
            new_map[k] = new_v
        if not changed:
    ...
    if isinstance(attr, ParametrizedAttribute):
        new_params = [resolve_transfer_widths(p, width_map) for p in attr.parameters]
...
class ResolveTransferWidthsPattern(RewritePattern):
    ...
    def match_and_rewrite(self, op: Operation, rewriter: PatternRewriter):
        for result in tuple(op.results):
            new_type = resolve_transfer_widths(result.type, self.width_map)
        ...
        for region in op.regions:
            for block in region.blocks:
                for arg in tuple(block.args):
                    new_type = resolve_transfer_widths(arg.type, self.width_map)
        ...
        for name, attr in op.attributes.items():
            new_attr = resolve_transfer_widths(attr, self.width_map)
            if new_attr != attr:
...
class ResolveTransferWidths(ModulePass):
    ...
    def apply(self, ctx: Context, op: ModuleOp):
        ...
        walker = PatternRewriteWalker(
            ResolveTransferWidthsPattern(width_map), walk_reverse=True
        )
