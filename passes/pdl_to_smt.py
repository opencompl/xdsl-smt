...
from xdsl.dialects.pdl import OperandOp, OperationOp, PatternOp, ReplaceOp, ResultOp, RewriteOp, TypeOp
...
def _get_type_of_erased_type_value(value: SSAValue) -> Attribute:
    ...
    assert isinstance((type_op := value.old_value.owner),
                      TypeOp), 'Error in rewriting logic'
...
class OperationRewrite(RewritePattern):
    ...
    def match_and_rewrite(self, op: OperationOp, rewriter: PatternRewriter):
        ...
        result_types = [
            _get_type_of_erased_type_value(type) for type in op.typeValues
        ]
        synthesized_op = op_def.create(operands=op.operandValues,
...
class ResultRewrite(RewritePattern):
    ...
    def match_and_rewrite(self, op: ResultOp, rewriter: PatternRewriter):
        ...
        result = self.rewrite_context.pdl_op_to_op[
            op.parent_.old_value].results[op.index.value.data]
